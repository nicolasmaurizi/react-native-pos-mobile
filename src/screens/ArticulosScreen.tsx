...
import { listArticulos, softDeleteArticulo, upsertArticulo } from "../features/articulos/articulos.repo";
...
export default function ArticulosScreen() {
  ...
  const refresh = () => setRows(listArticulos(search, true));
                ...
                <View style={styles.row}>
                  <View style={{ flex: 1, paddingRight: theme.space(1) }}>
                    ...
                    <View style={styles.metaRow}>
                      <Text style={styles.price}>{moneyARS(item.precio)}</Text>
                      {item.stock != null && (
                        <Text style={styles.meta}>Stock: {item.stock} {item.unidad ?? "u"}</Text>
                      )}
                    </View>
